...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Karma: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Karma Signed Message:\n";
...
{
    // Karma: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Karma: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Karma: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Karma: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KarmaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KarmaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KarmaMiner(CWallet *pwallet)
{
    printf("KarmaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("karma-miner");
    ...
    try { loop {
        ...
        printf("Running KarmaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KarmaMiner, pwallet));
}
