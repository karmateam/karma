...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Karmacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Karmacoin Signed Message:\n";
...
{
    // Karmacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Karmacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	if (fTestNet) {
		...
		if (pindexLast->nHeight+1 >= 10500) { DiffMode = 2; }
	}
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Karmacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Karmacoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "25/Jan/2014 Stephen Hawking: There are no black holes";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Karmacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KarmacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KarmacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KarmacoinMiner(CWallet *pwallet)
{
    printf("KarmacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("karmacoin-miner");
    ...
    try { loop {
        ...
        printf("Running KarmacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KarmacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KarmacoinMiner, pwallet));
}
